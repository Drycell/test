...
def test_ctrnn_deterministic_step_and_edits():
    ...
    c1.reset(seed=1)
    c2.reset(seed=1)
    sensor_obs = np.zeros(6, dtype=float)
    a1 = c1.step(sensor_obs)
    a2 = c2.step(sensor_obs)
    assert np.allclose(a1, a2)

    genome = StructuralGenome(
        edits=[
            StructuralEdit(op="add_chem", src=0, dst=1, value=1.0),
            StructuralEdit(op="flip_sign", src=0, dst=1),
...
def test_virtual_sensor_mapping_order():
    ...
    c.reset(seed=0)
    sensor_obs = np.array([1.0, 2.0, 3.0, 4.0, 6.0, 5.0], dtype=float)
    c.step(sensor_obs)
